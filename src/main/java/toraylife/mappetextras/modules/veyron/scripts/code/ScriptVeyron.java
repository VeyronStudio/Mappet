...
public class ScriptVeyron implements IScriptVeyron
{
    ...
    public IScriptVeyron setZone(String zone) { if (zone != null) this.state.zones.add(zone); return this; }
    public IScriptVeyron clearZone(String zone) { this.state.zones.remove(zone); return this; }
    ...
    public IScriptVeyron playMusic(String sound) { this.state.music = sound == null ? "" : sound; this.play(sound, 1.0F, 1.0F, this.player.posX, this.player.posY, this.player.posZ); return this; }
    public IScriptVeyron stopMusic() { this.state.music = ""; return this; }
    ...
    public IScriptVeyron reset() { this.state.fear = 0; this.state.tension = 0; this.state.visionLevel = 0; this.state.soundMuffle = 0.0F; this.state.speedMultiplier = 1.0F; this.state.zones.clear(); this.state.lockedActions.clear(); this.state.noises.clear(); this.state.jumpscares.clear(); this.state.music = ""; this.state.timers.clear(); this.state.screenDarkness = 0.0F; this.state.screenBlur = 0.0F; this.state.questHud = false; this.state.trackedQuest = ""; this.state.activeHud = ""; this.state.huds.clear(); this.player.removePotionEffect(MobEffects.BLINDNESS); this.player.removePotionEffect(MobEffects.NAUSEA); this.player.removePotionEffect(MobEffects.SLOWNESS); this.applySpeed(); VeyronManager.syncOverlay(this.player); return this; }
    ...
    private void play(String sound, float volume, float pitch, double x, double y, double z)
    {
        ...
        VeyronManager.playSound(this.player, event == null ? SoundEvents.AMBIENT_CAVE : event, x, y, z, volume, pitch);
    }
